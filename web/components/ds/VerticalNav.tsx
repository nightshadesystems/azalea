...
export interface VerticalNavItem {
  ...
  badgeStatus?: 'info' | 'success' | 'warning' | 'danger';
}
...
export interface VerticalNavProps {
  ...
  activeId?: string;
  onNavigate?: (item: VerticalNavItem) => void;
  className?: string;
...
export function VerticalNav({
  ...
  return (
    <nav
      className={['clr-vertical-nav', collapsed ? 'is-collapsed' : '', className].filter(Boolean).join(' ')}
      style={style}
    >
      {collapsible && (
        <button
        ...
        >
          <clr-icon shape="angle-double" dir={collapsed ? 'right' : 'left'} size="14"></clr-icon>
        </button>
      )}
      {groups.map((g, gi) => (
          ...
          {g.items.map((it) => (
            ...
            >
              ...
              {!collapsed && it.badge != null && (
                <span className={'badge nav-badge' + (it.badgeStatus ? ' badge-' + it.badgeStatus : '')}>
                  {it.badge}
                </span>
              )}
            </a>

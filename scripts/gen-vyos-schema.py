...
Azalea edits - the nineteen `interfaces <type>` trees and the NAT trees
(`nat`, `nat cgnat`, `nat64`, `nat66`) - into:

  web/lib/vyos-interfaces.generated.ts   full trees: help, values, ranges,
                                         regexes, defaults - drive the
...
def main() -> None:
    ...
    ts.write_text(
        ...
        + json.dumps(interfaces, separators=(",", ":"), ensure_ascii=False)
        + ";\n\n"
        "/** Other top-level config nodes Azalea edits: nat (with cgnat), nat64, nat66. */\n"
        "export const VYOS_ROOTS: Record<string, SchemaNode> = "
        + json.dumps({k: v for k, v in roots.items() if k != "interfaces"}, separators=(",", ":"), ensure_ascii=False)
        + ";\n",
        encoding="utf-8",
        newline="\n",
    )

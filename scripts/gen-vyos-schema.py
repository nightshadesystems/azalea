#!/usr/bin/env python3
"""Generate the interface configuration schema from vyos-1x.

VyOS describes its CLI in interface-definitions/*.xml.in (with #include
of interface-definitions/include/*.xml.i). This turns the nineteen
`interfaces <type>` definitions into:

  web/lib/vyos-interfaces.generated.ts   full tree: help, values, ranges,
                                         regexes, defaults - drives the
...
Usage:  python3 scripts/gen-vyos-schema.py /path/to/vyos-1x
The checkout's branch and commit are recorded in both outputs.
"""
...
def main() -> None:
    ...
    origin = f"{git('rev-parse', '--abbrev-ref', 'HEAD')} {git('rev-parse', '--short', 'HEAD')} ({git('log', '-1', '--format=%cs')})"

    schemas = {}
    for t in TYPES:
        text = resolve((defs / f"interfaces_{t}.xml.in").read_text(encoding="utf-8"), defs)
        root = ET.fromstring(text)
        tag = root.find(f"./node[@name='interfaces']/children/tagNode[@name='{t}']")
        if tag is None:
            sys.exit(f"{t}: no tagNode in definition")
        schemas[t] = parse_node(tag, t)
        print(f"{t:18} {count(schemas[t]):4} nodes", file=sys.stderr)

    ts = ROOT / "web" / "lib" / "vyos-interfaces.generated.ts"
    ts.write_text(
        "// GENERATED by scripts/gen-vyos-schema.py - do not edit.\n"
        f"// Source: vyos-1x {origin}, interface-definitions/interfaces_*.xml.in\n"
        ...
        + json.dumps(schemas, separators=(",", ":"), ensure_ascii=False)
        + ";\n",
        encoding="utf-8",
        newline="\n",
    )
    ...
    js.write_text(
        json.dumps({"source": origin, "types": {t: kinds_only(s) for t, s in schemas.items()}}, separators=(",", ":")),
        encoding="utf-8",
        newline="\n",
    )
    print(f"wrote {ts.relative_to(ROOT)} ({ts.stat().st_size // 1024} kB) and {js.relative_to(ROOT)} ({js.stat().st_size // 1024} kB)", file=sys.stderr)
